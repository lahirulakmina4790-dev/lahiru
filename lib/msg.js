...
const downloadMediaMessage = async(m, filename) => {
	...
	if (msgType === 'viewOnceMessage' || msgType === 'viewOnceMessageV2') {
		// View Once Message එකකදී, ඇතුළත ඇති සත්‍ය Message Type එක ගන්න
		msgType = msgObj.type
	}
	...
	if (msgType === 'imageMessage') {
		...
		const stream = await downloadContentFromMessage(msgObj, 'audio')
		let buffer = Buffer.from([])
		for await (const chunk of stream) {
			buffer = Buffer.concat([buffer, chunk])
		}
		...
		const stream = await downloadContentFromMessage(msgObj, 'sticker')
		let buffer = Buffer.from([])
		for await (const chunk of stream) {
			buffer = Buffer.concat([buffer, chunk])
		}
		...
		var ext = msgObj.fileName.split('.')[1].toLowerCase().replace('jpeg', 'jpg').replace('png', 'jpg').replace('m4a', 'mp3')
		var nameDoc = filename ? filename + '.' + ext : 'undefined.' + ext
		const stream = await downloadContentFromMessage(msgObj, 'document')
		let buffer = Buffer.from([])
		for await (const chunk of stream) {
			buffer = Buffer.concat([buffer, chunk])
		}
...
const sms = (zanta, m) => {
	...
	if (m.message) {
		...
		if (m.type === 'viewOnceMessage' || m.type === 'viewOnceMessageV2') {
			// View Once නම්, inner message එකේ object එක ගන්න
			const innerMessage = m.message[m.type].message;
			m.msg = innerMessage[getContentType(innerMessage)];
		} else {
		...
		if (m.msg) {
			...
			if (m.type === 'viewOnceMessage' || m.type === 'viewOnceMessageV2') {
				// View Once එකේ ඇතුළත ඇති සත්‍ය Message Type එක set කරන්න
				const innerMessage = m.message[m.type].message;
				m.msg.type = getContentType(innerMessage);
			}
